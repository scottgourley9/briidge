...
const Operators = ({
    ...
    const renderCategories = useMemo(() => {
        ...
        return a.map((v, i) => {
            return (
                <div key={`${v}${i}`}>
                    <input disabled={isLoading} type="checkbox" id={`${v}${i}`} value={v} onChange={e => handleFilter(e, categoryFilter, 'operating_category', updateCategoryFilter)} /> <label htmlFor={`${v}${i}`}>{v}</label>
                </div>
            )
        });
    ...
    const renderTimeframe = useMemo(() => {
        ...
        return a.map((v, i) => {
            return (
                <div key={`${v}${i}`}>
                    <input disabled={isLoading} type="checkbox" id={`${v}${i}`} value={v} onChange={e => handleFilter(e, timeframeFilter, 'timeframe', updateTimeframeFilter)} /> <label htmlFor={`${v}${i}`}>{v}</label>
                </div>
            )
        });
